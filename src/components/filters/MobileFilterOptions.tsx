...
const MobileFilterOptions: React.FC = () => {
  ...
  return <div className="grid grid-cols-1 gap-0">
      ...
      <FilterSectionComponent title="Origem" isExpanded={expandedSections.origin || false} onToggle={() => toggleSection('origin')}>
        <FilterDropdown id="origin-filter-mobile" aria-label="Selecionar origem" value={filters.origin} onChange={handleOriginChange} options={originOptions} className="border-gray-200 shadow-sm" />
      </FilterSectionComponent>

      <FilterSectionComponent title="Etapa" isExpanded={expandedSections.place || false} onToggle={() => toggleSection('place')}>
        <FilterDropdown id="place-filter-mobile" aria-label="Selecionar etapa" value={filters.place} onChange={handlePlaceChange} options={placeOptions} className="border-gray-200 shadow-sm" />
      </FilterSectionComponent>
    </div>;
};
