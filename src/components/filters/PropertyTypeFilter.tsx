...
const PropertyTypeFilter: React.FC<PropertyTypeFilterProps> = ({
  ...
  return <fieldset className="space-y-4">
      ...
      <RadioGroup className="flex flex-wrap gap-2" value={currentValue} onValueChange={handlePropertyTypeChange}>
        {availableTypes.map(type => <div key={`${id}-${type}`} className="relative flex flex-col items-start gap-2 rounded-lg border border-input p-2 shadow-sm shadow-black/5 has-[[data-state=checked]]:border-purple-300 has-[[data-state=checked]]:bg-purple-50">
