...
import { Car, Home, Plane, Ship, Truck, Tractor, Bike, CircleDashed, Building, Mountain, Hotel, Warehouse, Caravan, Forklift } from 'lucide-react';
...
const CategoryFilter: React.FC<CategoryFilterProps> = ({
  ...
  let categoryOptions = contentType === 'property' ? getPropertyCategories() : getVehicleCategories();
  ...
  return <div className="space-y-3">
      <RadioGroup value={category || ""} onValueChange={handleCategoryChange} aria-label="Selecione uma categoria" className="grid grid-cols-3 gap-2 w-full">
        {categoryOptions.map(option => {
        const Icon = getIconForCategory(option);
        return <div key={`${id}-${option}`} className="relative flex flex-col gap-2 rounded-lg border border-gray-200 p-2 shadow-sm shadow-black/5 has-[[data-state=checked]]:border-blue-300 has-[[data-state=checked]]:bg-blue-50">
