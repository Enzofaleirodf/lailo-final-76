...
const FilterSection: React.FC<FilterSectionProps> = ({
  ...
  const footerContent = <div className="flex gap-3 p-4 bg-white border-t border-gray-200">
      <Button variant="outline" className="flex-1 h-10 bg-white border-gray-300 hover:bg-gray-50 hover:text-gray-700" onClick={() => handleOpenChange(false)} aria-label="Cancelar e fechar filtros">
        Cancelar
      </Button>
      <FilterApplyButton contentType={contentType} onApply={handleApplyFilters} className="flex-1" />
    </div>;
  ...
  return <Drawer open={filtersOpen} onOpenChange={handleOpenChange}>
      <DrawerContent className="p-0 overflow-hidden h-[98vh] max-h-[98vh]" footerContent={footerContent} isOpen={filtersOpen} showHandle={false}>
        <div className="flex flex-col h-full">
          <div className="sticky top-0 z-[201] flex justify-between items-center p-3 bg-brand-600 text-white border-b border-brand-700 rounded-t-lg">
            ...
            <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-white hover:bg-brand-700 flex items-center justify-center" onClick={() => handleOpenChange(false)} aria-label="Fechar filtros">
              <X size={18} />
              <span className="sr-only">Close</span>
            </Button>
          </div>
          
          <div aria-labelledby="drawer-title" className="bg-gray-50 p-3 flex-1 overflow-y-auto" role="region">
            <FilterContent contentType={contentType} />
          </div>
        </div>
      </DrawerContent>
