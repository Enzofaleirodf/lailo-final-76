    ...
    <motion.div className="mb-2">
      <div className="flex p-3 rounded-xl bg-white border border-[#E5E5E5] shadow-sm">
        ...
        <div className="flex-1 pl-3 flex flex-col">
          <Skeleton className={`${!isMobile ? 'h-[18px]' : 'h-5'} w-4/5 mb-2`} />
          <Skeleton className={`${!isMobile ? 'h-[15px]' : 'h-4'} w-2/3 mb-3`} />
          
          <Skeleton className={`${!isMobile ? 'h-[18px]' : 'h-5'} w-24 mt-2`} />
          
          <div className="border-t border-[#E5E5EA] mt-4 mb-4"></div>
          
          <div className="flex justify-between items-center mt-auto">
            <div className="flex gap-2">
              <Skeleton className={`${!isMobile ? 'h-[14px]' : 'h-5'} w-16`} />
              <Skeleton className={`${!isMobile ? 'h-[14px]' : 'h-5'} w-16`} />
            </div>
            <Skeleton className={`${!isMobile ? 'h-[14px]' : 'h-5'} w-24`} />
          </div>
        </div>
      </div>
    </motion.div>
